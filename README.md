...
- **智能回放** — 精确还原录制事件，支持循环次数和自定义延时
- **图像识别** — 基于 OpenCV 模板匹配，自动查找目标并点击
- **防检测** — 随机延时、随机偏移、线性插值鼠标移动，降低检测风险
- **轻量 UI** — 基于 Dear ImGui 的简洁界面，支持录制/回放/设置管理
...
1. 运行 `ShadowKey.exe`
2. 点击 **Start Recording**，执行你希望录制的操作
3. 点击 **Stop Recording** 完成录制
...
在 `.sks` 文件中为动作添加 `image_trigger` 字段：
